...
static uint32_t CRC32Target = 0;
static uint32_t CRC32Reference = 0;
...
void InitCRC32(void)
{
	CRC32Target = Set_CRC32InitValue(CRC_INIT_VALUE);
	CRC32Reference = Set_CRC32InitValue(CRC_INIT_VALUE);
}


uint32_t Get_CRC32(unsigned char newData, uint32_t oldData)
{
	return CRC32_TABLE[(oldData >> 24) ^ newData] ^ (oldData << 8);
	//CRC32_TABLE[(cksum_temp1 >> 24) ^ SPI_Buffer[Idx]]     ^ (cksum_temp1 << 8)
}
